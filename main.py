...
# Allow browser-based clients (web testers, frontends) to call this API.
# Without this, any fetch() from a page on a different origin fails with a
# CORS error that shows up in the browser simply as "Failed to fetch".
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# Certifi bundle and OS trust store both failed to validate Pinterest's
# cert chain on this host (broken/incomplete chain server-side, not fixable
# from the client). Bypass verification directly instead.
