...
- `POST /api/search` — `{"query": "naruto", "type": "image"|"video", "limit": 10}` — max **20** for images, max **5** for videos per call (hard-capped server-side regardless of what's requested, to protect against abuse/Render disk/suspension risk)
...
## Disk safety
Every downloaded file auto-deletes right after it's sent (per-request cleanup).
As a backup, a background sweep also runs every 2 minutes and removes anything
left in `downloads/` older than 10 minutes — so even a crashed/killed request
can't leave files piling up and filling Render's disk.
